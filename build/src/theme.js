...
$(document).ready(function () {
    ...
    if (window.concept) {
        ...
        // themeRes = themeRes.replace(/&quot;+/g, '"').replace(/\"{/g,'{').replace(/}\"/g,'}').trim();
        ...
        var stockUrl = STOCKURL + 'concept=' + concept + '&date=' + dateStr + '&src=*&period=10&response=application/json';
...
function _renderCharts(heatUrl, stockUrl) {
    ...
    $.ajax({
        ...
        success: function success(data) {
            if (data) {
                //render line chart
                _renderLineChart(lineChart, JSON.parse(data));
            } else {
                //show message
            }
        },
...
function _renderHistogram(data) {
    ...
    for (var i = 0, len = legend.length; i < len; ++i) {
        ...
        if (chartData.length && !isEmpty) {
            ...
            for (var n = 0; n < l; ++n) {
                if (n % 2) {
                    _renderRightHistogram(chartData[i].category, chartData[i].data[n], chartIndex[n], chartColor[n], id);
                    echarts.connect([histogram[n - 1 + l * i], histogram[n + l * i]]);
                    console.log(n - 1 + l * i, n + l * i);
                } else {
                    _renderLeftHistogram(chartData[i].category, chartData[i].data[n], chartIndex[n], chartColor[n], id);
                }
            }
        } else {
            //show message
