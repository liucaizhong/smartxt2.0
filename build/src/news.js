...
$(document).ready(function () {
	$('#text-search').change(function (event) {
...
function onFilterChange(that, event) {
	...
	if (target.tagName === 'DD') {
		...
		searchCondition[whichCondition].forEach(function (cur, index, arr) {
			...
			if (cur.key.substr(1, 1) == '1') {
				...
				if (cur.key != key) {
					...
					if (arr.length == 1) {
						var id = cur.key.substr(0, 1) + '1';
						var _value = $(id).attr('data-value');
						$('#' + cur.key).removeClass('active');
						$('#' + id).addClass('active');
						arr.splice(index, 1, {
							key: id,
